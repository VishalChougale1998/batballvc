...
app.post("/api/create-league", upload.single("banner"), async (req, res) => {
    try {
        ...
        const league = await League.create({
            ...
            entryFee: Number(String(entryFee).replace(/[^\d]/g, "")) || 0,
            lastDate: lastDate ? new Date(lastDate) : null,
            banner: req.file ? req.file.path : "", // ✅ Cloudinary URL
        });
...
app.post("/api/teams", async (req, res) => {
    try {
        const cleanPurse = Number(String(req.body.purse).replace(/[^\d]/g, "")) || 50000;
